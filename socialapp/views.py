...
def topic_detail_view(request, slug):
    ...
    if request.method == 'POST':
        ...
        if new_comment.is_valid():
            ...
            messages.error(
                request,
                'Oops! There was an error and your comment could not be posted!'
                )
...
def updateTopic(request, slug):
    ...
    if request.method == 'POST':
        ...
        if selected_topic.is_valid():
            ...
            messages.error(
                request,
                'Oops! Your topic could not be updated. Please ensure the slug is unique as it may already be in use. Please submit a ticket if further issues.')
    context = {
